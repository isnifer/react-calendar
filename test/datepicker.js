...
test('Calendar inited', assert => {
  ...
  const DOMComponent = TestUtils.renderIntoDocument(<Datepicker initialDate={initialDate} />)
...
test('Should have 3 elements: head, month, placeholder for nav', assert => {
  ...
  assert.equal(
    ...
    'calendar__head',
    'And should have className calendar__head'
  )
  ...
  assert.equal(
    ...
    'calendar__month',
    'And should have className calendar__month'
  )
...
test('Test componentWillReceiveProps', assert => {
  ...
  let currentDay1 = TestUtils.findRenderedDOMComponentWithClass(
    datePicker1,
  ...
  let currentDay2 = TestUtils.findRenderedDOMComponentWithClass(
    datePicker2,
    'calendar__day_current'
  )
  // TEMP HACK
  ...
  let nextAvialableDate = TestUtils.scryRenderedDOMComponentsWithClass(
    datePicker1,
    'calendar__day_available'
  )[1]
